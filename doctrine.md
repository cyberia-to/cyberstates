...
taxation — and every domain is held separately: kept local, inherited
...
Citizenship is a position: most humans hold at least one, and the
...
Movement is only the first domain so quoted. The freedom to hold and
move capital, to own land as a foreigner, to speak, to leave — each is
an option over the same underlying, measurable in the same shape. The
...
roll today counts 271: UN members, their autonomous territories,
...
[Antarctica](/state/aq), and beyond the atmosphere the forty
solid-surface bodies of the solar system with the [Sun](/state/sun)
itself, listed for its Dyson shell. From the [Moon](/state/luna) to
[Sedna](/state/sedn) they hold territory the way Antarctica does —
under a treaty instead of a throne — with population zero and capital
zero: the supply curve of unmonetized space, quoted before the rules
arrive. The same test admits what comes next: as
...
*Cyberstates is this doctrine, instrumented: every thesis above is a
column in the table.*
...
The geometric mean, deliberately: both dimensions must be real (√(80×20)
= 40, where the arithmetic mean would flatter with 50); zero is
contagious (a passport admitted nowhere scores 0, not half of the other
