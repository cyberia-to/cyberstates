            ...
            <div class="cyberia-stage">
                ...
                <section class="cyberia-panel cyberia-fleets">
                                ...
                                <button
                                    class=move || {
                                        ...
                                        format!(
                                            ...
                                            if f.kind == "worker" { " worker" } else { " machine" },
                                        )
                                    }
                                    disabled=offline
                                    on:click=move |_| {
                    ...
                    <div class="cyberia-hint">
                        "Pick a fleet unit, a flat on the map, an action — then commit intent."
                    </div>
                </section>
                            ...
                            view! {
                                <svg class="flat-map" viewBox=format!("0 0 {W} {H}") preserveAspectRatio="xMidYMid meet">
                                    ...
                                    {m.phase0.iter().map(|flat| {
                                        ...
                                        let label = flat.name.to_uppercase();
                                        view! {
                                            <g class="flat-poly"
                    ...
                    <div class="intent-form">
                        ...
                        <button
                            ...
                            on:click=move |_| {
                                ...
                                // soft3: offline fleets cannot take intent
                                ...
                                intents.update(|q| {
                                    q.insert(0, Intent {
                                        id,
                                        fleet: FLEETS.iter().find(|f| f.id == fleet).map(|f| f.name.to_string()).unwrap_or(fleet),
                                        action,
                                        flat,
                                    });
                                });
                        ...
                        <div class="intent-queue">
                            {move || {
                                ...
                                if q.is_empty() {
                                    return view! {
                                        <div class="intent-empty">"no intents yet — assign a fleet to a flat"</div>
                                    }.into_any();
                                }
                                view! {
                                    <div class="intent-list">
                                        {q.into_iter().take(8).map(|it| {
                                            view! {
                                                <div class="intent-item">
                                                    <span class="ii-id">{format!("#{:03}", it.id)}</span>
                                                    <span class="ii-fleet">{it.fleet}</span>
                                                    <span class="ii-act">{it.action.to_uppercase()}</span>
            ...
            <div class="search-dock cyberia-dock">
                <span class="dock-count">
                    {move || {
                        ...
                        format!("{n} intents · {m} phase-0 flats · {} fleets", FLEETS.len())
                    }}
                </span>
                <span class="dock-credit cyberia-soft3-dock">
                    "soft3-compliant experimental surface · open map · local intent queue · no closed backend"
                </span>
